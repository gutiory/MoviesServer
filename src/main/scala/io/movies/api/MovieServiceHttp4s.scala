...
class MovieServiceHttp4s[F[_]: Sync: MovieService] extends Http4sDsl[F] {
  val ms: MovieService[F]                                           = implicitly[MovieService[F]]
  implicit val decoder: EntityDecoder[F, Movie]                     = jsonOf[F, Movie]
  implicit val encoder: EntityEncoder[F, RegisteredMovie]           =
    jsonEncoderOf[F, RegisteredMovie]
  implicit val encoderList: EntityEncoder[F, List[RegisteredMovie]] =
    jsonEncoderOf[F, List[RegisteredMovie]]
  ...
  val service: HttpRoutes[F] = HttpRoutes.of[F] {
    case GET -> root / "movie" / id       => {
      ms.getMovieById(id.toShort)
        .flatMap(_.fold(NotFound(s"No movie found with ID $id"))(Ok(_)))
    }
    ...
    case request @ POST -> root / "movie" => {
      (for {
        ...
        res   <- Created(rm)
      } yield res).handleErrorWith(_ =>
        InternalServerError("Movie could not be inserted")
      )
    }
