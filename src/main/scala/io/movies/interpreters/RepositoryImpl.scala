...
object RepositoryImpl {
  ...
  )(implicit S: Sync[F], logger: SelfAwareStructuredLogger[F]): Repository[F] =
    new Repository[F] {
      override def addMovie(movie: Movie): F[RegisteredMovie] = {
        val insert: ConnectionIO[RegisteredMovie] =
          sql"""insert into movie (title, director, year)
              values (${movie.title}, ${movie.director}, ${movie.releaseDate.getValue})""".update
            .withUniqueGeneratedKeys[RegisteredMovie](
              "id",
              "title",
              "director",
              "year"
      ...
      override def getMovies: F[List[RegisteredMovie]] = {
        val select = sql"select id, title, director, year from movie"
          .query[RegisteredMovie]
          .to[List]
      ...
      override def getMovieById(id: Int): F[Option[RegisteredMovie]] = {
        val select =
          sql"select id, title, director, year from movie where id = $id"
            .query[RegisteredMovie]
            .option
